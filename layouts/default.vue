          ...
          <div class="router-content">
            ...
            <transition :name="routerTransition">
              ...
              <div v-if="$route.meta.breadcrumb || $route.meta.pageTitle" class="flex flex-wrap items-center mb-6 router-header">
                ...
                <!-- BREADCRUMB -->
                <vx-breadcrumb class="hidden ml-4 md:block" v-if="$route.meta.breadcrumb" :route="$route" :isRTL="$vs.rtl" />
...
export default {
  ...
  computed: {
    ...
    document(){
      let doc
     if (process.client) {doc = document}else{doc = {}}
     return doc
    },
     window(){
      let win
     if (process.client) {win = window}else{win = {}}
     return win
    },
  ...
  methods: {
     toggleClassInBody(className) {
      if (className == 'dark') {
        if (this.document.body.className.match('theme-semi-dark')) this.document.body.classList.remove('theme-semi-dark')
        this.document.body.classList.add('theme-dark')
      }
      else if (className == 'semi-dark') {
        if (this.document.body.className.match('theme-dark')) this.document.body.classList.remove('theme-dark')
    ...
    handleWindowResize() {
      ...
      // Set --vh property
      this.document.documentElement.style.setProperty('--vh', `${this.window.innerHeight * 0.01}px`);
    },
    handleScroll() {
